...
class _NewHitaGradientCircularProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (backgroundColor != Colors.transparent) {
      ...
      canvas.drawArc(rect, 0, total, false, paint);
    }
    ...
    canvas.rotate(_start);
    paint.color = Colors.white;
    paint.strokeCap = StrokeCap.round;
    paint.shader = null;
    canvas.drawPoints(
        PointMode.points, [Offset((size.width / 2) - 1, 0)], paint);
    paint.color = colors[0];
    canvas.drawCircle(Offset((size.width / 2) - 1, 0), 3, paint);
    // canvas.restore();
  }
