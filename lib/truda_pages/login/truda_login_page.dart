...
class TrudaLoginPage extends StatefulWidget {
  TrudaLoginPage({Key? key}) : super(key: key);

  @override
  State<TrudaLoginPage> createState() => _TrudaLoginPageState();
}

class _TrudaLoginPageState extends State<TrudaLoginPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<TrudaLoginController>(builder: (controller) {
      return Scaffold(
  ...
  /// 登录
  Widget _appleLogin(TrudaLoginController controller) {
  ...
  Widget _accountLogin() {
    return GetPlatform.isIOS == true
        ...
        : GestureDetector(
            onTap: () {
              Get.to(const TrudaAccountLoginPage());
            },
            child: Container(
              width: double.infinity,
              height: 48,
  ...
  /// 快捷登录
  Widget _visitorLogin(TrudaLoginController controller) {
    return GetPlatform.isIOS == true
        ? const SizedBox()
        : GestureDetector(
            onTap: () {
