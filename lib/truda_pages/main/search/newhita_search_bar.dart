...
/// 搜索AppBar
class NewHitaAppBarSearch extends StatefulWidget implements PreferredSizeWidget {
  NewHitaAppBarSearch({
    Key? key,
  ...
  @override
  _NewHitaAppBarSearchState createState() => _NewHitaAppBarSearchState();

  @override
  Size get preferredSize => Size.fromHeight(height);
}

class _NewHitaAppBarSearchState extends State<NewHitaAppBarSearch> {
  ...
  List<Widget> _actions() {
    ...
    list.add(InkWell(
      onTap: () {
        var te = _controller?.text ?? '';
        if (te.isEmpty) return;
        widget.onRightTap?.call(_controller?.text ?? '');
        _focusNode?.unfocus(); //失去焦点
      },
      child: Container(
        ...
        child: Text(
          ...
          // fontColor: TrudaColors.textColor666,
          // fontSize: 14,
        ),
      ),
    ));
